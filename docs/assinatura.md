...
> **Nota operacional — quem assina em cada etapa (autorização por SETOR, não pelo campo "Vendedor"):** a autorização é pelo **setor da conta logada**, não pelo *vendedor responsável* exibido na prova. Por isso um usuário **3Studio** que escaneia uma prova **Matriz/Filial** ainda em **"Criada"** vê o bloqueio "Esta prova não está aguardando uma ação sua" — nessas rotas o primeiro a agir é o **Vendedor** (*Retirada/Encaminhada para o Vendedor*); o 3Studio só age depois (em "Aprovada pelo Vendedor"). Já em **Lam. Matriz / Lam. Filial** o **3Studio é o primeiro** a agir na "Criada" (*Encaminhada para Laminação*). Para testar a assinatura com um só operador, use uma rota com laminação **ou** entre com uma conta de setor Vendedor.
