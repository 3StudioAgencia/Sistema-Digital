...
- **Inferior:** **RLS** do PostgreSQL, políticas versionadas em `apps/api/migrations/rls/` (uma por perfil × tabela sensível). *(W1-C05)* `usuarios` com RLS por perfil + **helpers** `app_setor()`/`app_is_admin()`/`app_current_user_id()` que o **C06** compõe em `provas`; o backend honra a RLS propagando claims por requisição (ADR-008/ADR-031: `SET LOCAL ROLE authenticated` + `request.jwt.claims`). Espelho de domínio em `apps/api/src/domain/rbac.py`.
...
>
> **RBAC (W1-C05):** Matriz §7 = fonte única em **`apps/web/src/lib/access-matrix.ts`** (`can`/`podeAcessarRota`) + RLS em **`apps/api/migrations/rls/`** (helpers `app_setor`/`app_is_admin`/`app_current_user_id` que o C06 usa em `provas`). **Regra do PR único:** mudança na Matriz cobre `access-matrix.cells.json` + `access-matrix.ts` + `domain/rbac.py` + as migrations de RLS — junto. Migrations `0004` (hook) + `0005` (RLS de `usuarios`); habilitar o hook no dashboard (Auth → Hooks → `public.custom_access_token_hook`). Proxy decide com **`getClaims()`** (local). Testes RBAC (@db; mesma `TEST_DATABASE_URL`): `uv run pytest tests/integration/test_custom_access_token_hook.py tests/integration/test_rls_usuarios.py tests/integration/test_claims_propagation.py tests/unit/test_equivalencia_matriz.py`; harness de equivalência web em `pnpm test` (`access-matrix.equivalencia.test.ts`). Detalhes em `docs/rbac.md`.
