...
- **Emenda (W1-C03, refino):** adicionado **`SPRING`** a `tokens.ts` (molas `interactive` e `parallax`) — extensão do DAT §5.1 para física de microinteração. Para **harmonia**, todas as interações (hover/press de botões e do herói + parallax) usam a MESMA mola; o **contorno de foco dos campos faz fade-in/out** (overlay só de `opacity` — GPU, em vez de troca instantânea de borda). Mantida a semântica DAT: entradas `emphasized`, saídas `exit`, fades `standard`; stagger único (0.07/0.06).
