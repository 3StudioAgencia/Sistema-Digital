...
| Dashboard: RF-015 (Must) pede 6 contadores de "Operação Diária" + 3 de "Em Trânsito" | — | **Design exato — 5 contadores** (W4-C16/DP-1 — ADR-080): o desenho mostra só **Criadas hoje, Com Vendedor, Aprovadas, Na clicheria, Atrasadas** e o dono optou por **segui-lo**, omitindo conscientemente "Reprovadas" e o bloco "Em Trânsito" do RF-015. **Reversível sem reescrita** (estender `domain/dashboard.py` + a query). "Na clicheria" = `recebida_clicheria` (= "Concluídas" do RF-015) |
...
### 5.5 Camada de animação (W6-C19 — entregue)

Tokens em `apps/web/src/lib/motion/tokens.ts` (`DURATION`, `EASING`, `SPRING`, `STAGGER`, `REVEAL_OFFSET`). **Proibido literais inline de duração/easing.** Hook **`useReducedMotion`** central (`lib/motion/hooks.ts`) zera tudo sob `prefers-reduced-motion` — **fonte única** (RN-012/RNF-010). **Fábricas de variants** em `lib/motion/variants.ts` (`fadeRise`/`staggerContainer`, recebem `reduced`). **Primitivas** em `components/ui/motion/`: `<PageTransition>` (RF-023 — transição de página, keyed por `pathname` no AppShell, **enter-only** — ADR-098), `<Reveal>`, `<Stagger>`/`<StaggerItem>` (cascata "elementos surgindo"); + `<MotionModal>` (`components/ui/modal`, **250 ms/RF-024 + `mode="wait"`**), `<AnimatedCounter>` (RF-025), `ToastProvider` (RF-027, erros ≥ 4 s). **Contenção:** só `transform`/`opacity` (GPU), `mode="wait"` nas saídas, durações nos tokens. **Duas técnicas** de aplicação por tela: **(A)** fábricas nos nós existentes (grids/tabelas — **cap `STAGGER.maxItens`=12**) e **(B)** primitivas (pilhas verticais). **Não** re-animar componentes auto-animados (`<AnimatedCounter>`, `<ProofTimeline>`). Detalhes: `docs/animations.md` (ADR-097..099).
...
- **Wave 5 — Relatórios/UX:** `17` Relatórios · ~~`18` Atalhos~~ **descartado por decisão de produto (ADR-095)**
- **Wave 6 — Animações/Auditoria:** `19` Camada de animações ✅ · `20` Log de auditoria (UI) ✅ — **Wave 6 e backlog v1.0 CONCLUÍDOS**
...
>
> **Máquina de estados (W3-C11 — o coração do domínio):** a Requisitos v1.0 **§6** vive em **código** (`apps/api/src/domain/state_machine/`), nunca no banco. **`rules.py`** = `TRANSITION_RULES` imutável indexada por `(rota, estado_atual) → tupla de Transicao` (a §6 inteira; `CANCELAR` em todo estado ativo; sem wildcard — não-listada → 422); **`machine.py`** = `avaliar_transicao` pura (indefinida → **422**, perfil errado → **403** genérico sem revelar o ator, motivo ausente → **422**); **`enums.py`** = `Acao` (↔ `acao_enum`) + `Autorizacao` (setor **ou** flag admin). Autorização: normal-flow pelo **setor**; **Cancelar/Reiniciar pela flag `administrador`** (ADR-064). **Serviço `application/transicoes.py`** (`ProvasTransicaoService.executar`): **atômico** (RNF-017) e **idempotente** (RNF-015/DP-2) — lock pessimista `FOR UPDATE` → `idempotency_key` → valida → `status` (+ `finalizada_em` nos terminais) + 1 `movimentacao` → commit; reenvio converge (200), chave reusada → **409**. **Endpoint `POST /provas/{id}/transicoes`** → `ProvaDetalheOut` (gate universal `Recurso.ESCANEAR`; `TransicaoIn{acao, assinatura_ref, idempotency_key, motivo?}` — `assinatura_ref` é stub no C11, o **C12** fornece a real + FK — DP-1). **Tabela `movimentacoes`** (migration **`0015`**) = **log de auditoria imutável** (DP-3/ADR-061 — **não** há `audit_log` separado): append-only por trigger `trg_movimentacoes_append_only` (bloqueia UPDATE/DELETE até para o owner) + sem GRANT; RLS SELECT espelha `provas` (`EXISTS`), INSERT `WITH CHECK` (ator=self + prova em escopo); `idempotency_key` UNIQUE; `acao_enum` sincronizado com `Acao`. **`provas` ganhou UPDATE** (só `status`/`finalizada_em`/`updated_at` — privilégio mínimo) + 5 policies `provas_update_*`. **RLS do Motorista AMPLIADA** (ADR-065): além dos 3 "Em Trânsito", os 3 estados de ORIGEM das suas transições (`encaminhada_para_laminacao`/`laminacao_concluida`/`de_volta_studio`) — `ESTADOS_ESCOPO_MOTORISTA` derivado da §6 (sem lógica de rota — §11); divergência da §7 registrada. Fronteiras: assinatura=C12, timeline=C13, cancelar/reiniciar+`ciclo_atual`=C14/C15 — todos **invocam** este motor. Testes: `uv run pytest tests/unit/test_state_machine.py tests/unit/test_transicao_service.py tests/integration/test_transicoes_endpoints.py tests/integration/test_rls_movimentacoes.py` (@db, mesma `TEST_DATABASE_URL`). Detalhes em `docs/maquina-estados.md`.
>
> **Assinatura digital (W3-C12 — fecha o laço do fluxo):** o comprovante DESENHADO de cada movimentação (RN-003). **Tabela `assinaturas`** (migration **`0016`**, **aplicada no Supabase real** — `alembic_version=0016`): imagem em **`bytea`** (DP-2 — nasce na MESMA transação da movimentação; **não** R2: não há URL pré-assinada e um PUT não participa do rollback), `prova_id`/`ator_id` denormalizados, **append-only** (trigger `trg_assinaturas_append_only` + sem GRANT UPDATE/DELETE), **RLS** espelha `provas` (`assinaturas_*.sql`: SELECT por `prova_id` visível, INSERT `WITH CHECK` ator=self+prova em escopo). A coluna **`movimentacoes.assinatura_ref` virou FK** → `assinaturas.id` (nullable — DP-1; o C11 a deixou stub). **`TransicaoIn` agora carrega `assinatura`** (PNG base64, não mais `assinatura_ref`): `ProvasTransicaoService.executar(assinatura_imagem)` valida (magic bytes — `domain/assinaturas.py`) → INSERE `assinaturas` → INSERE `movimentacoes` com a FK → 1 commit (atômico; reenvio idempotente não recria). **`GET /provas/{id}/acoes-disponiveis`** (`ProvasTransicaoService.acoes_disponiveis`) reusa `transicoes_de`+`autoriza` do C11 e devolve só as ações do fluxo de escaneamento (`ACOES_FLUXO_ESCANEAMENTO` — Cancelar/Reiniciar excluídos; lista vazia = não é a vez do ator, sem revelar quem — RN-014). UI **`/provas/[id]/confirmar`** — **redesenhada (ADR-096, re-skin fora do backlog):** card branco único (nome + badge de requerimento + metadados Cliente/Vendedor/Rota/Ciclo/Criada em/Status) com a **caixa de assinatura cinza-clara** (guias **X + linha + "Assine no espaço acima da linha"** ATRÁS do **canvas transparente** — o PNG passou a ter fundo transparente, DP-5) e **rodapé** disclaimer "Ao confirmar, você aprova as cores da prova digital" + **Cancelar** (= voltar) + **Confirmar assinatura** (escuro, com check); **Reprovar** = perigo no rodapé. **Comportamento idêntico ao C12** (3 branches: assinatura automática / Aprovar-Reprovar+motivo / bloqueio; `react-signature-canvas@1.1.0-alpha.2` — React 19; resiliência por idempotência+sessionStorage; `lib/api/transicoes.ts`; `error.tsx`). Testes: `uv run pytest tests/unit/test_assinaturas_dominio.py tests/unit/test_transicao_service.py tests/integration/test_transicoes_endpoints.py tests/integration/test_rls_assinaturas.py` (@db, mesma `TEST_DATABASE_URL`). Detalhes em `docs/assinatura.md`.
>
> **Timeline visual (W3-C13):** o componente **`<ProofTimeline provaId />`** (`apps/web/src/app/(app)/provas/[id]/_components/`) preenche a seção "Histórico de movimentações" do detalhe (C08), substituindo o empty state. **`GET /provas/{id}/movimentacoes`** (prefixo real **sem `/api`**) → `TimelineOut{rota, estado_atual, ciclo_atual, criada_em, etapas_canonicas, movimentacoes[]}` por **`ProvasConsultaService.obter_movimentacoes`** (gate universal `Recurso.PROVAS` — **mesmo escopo/404 genérico do detalhe**; RLS de `movimentacoes`). **Caminho canônico (DP-1):** **`sequencia_canonica(rota)`** em `domain/state_machine/machine.py` (+ `ACOES_AVANCO`) **derivado de `TRANSITION_RULES`** — a §6 NÃO é duplicada no front (fonte única; teste de consistência em `test_state_machine.py`). **Ciclo (DP-3):** agrupa por `movimentacoes.ciclo` (coluna já existente do 0015 — **sem retroação**). **Nome do responsável (DP-2b):** migration **`0017`** = **`private.nomes_de_usuarios(uuid[])`** SECURITY DEFINER (generaliza `nomes_de_vendedores` p/ qualquer setor; escopo do chamador re-aplicado; `migrations/rls/nomes_de_usuarios.sql`). **Assinatura (DP-2c):** só o selo `tem_assinatura` — a imagem `bytea` não trafega. Render puro em **`lib/provas/timeline.ts`** (`construirTimeline` — etapas percorrida/atual/futura + eventos reprovação/cancelamento/reinício); reusa `rotuloStatus` (C07)/`rotuloRota` (C08); animação GPU com `prefers-reduced-motion`; falha do histórico é isolada (não derruba o detalhe). Testes: `uv run pytest tests/unit/test_state_machine.py tests/unit/test_provas_timeline.py tests/integration/test_provas_timeline_endpoints.py tests/integration/test_rls_nomes_de_usuarios.py` (@db, mesma `TEST_DATABASE_URL`) + web `pnpm test`. **Migration `0017` aplicada no Supabase real** (`alembic_version=0017`). Detalhes em `docs/timeline.md`.
>
> **Cancelamento (W3-C14):** ação ADMINISTRATIVA de cancelar uma prova (3Studio, qualquer estado ativo, motivo obrigatório) — **terminal e irreversível** (RN-005). **Passa pelo motor do C11** (fonte única de status — ADR-073/062): endpoint **DEDICADO `POST /provas/{id}/cancelar`** (`CancelarIn{motivo, idempotency_key}` → `ProvaDetalheOut`, prefixo real **sem `/api`**) por **`get_cancelamento_service`** que gateia na **borda** por **`Recurso.CANCELAR_PROVA`** (flag admin) e invoca o MESMO `ProvasTransicaoService.executar(acao=CANCELAR, assinatura_imagem=None, motivo)` → grava 1 `movimentacao` (ator+data/hora+motivo) no log imutável, `status=cancelada`+`finalizada_em`. **Acesso em 2 camadas** (borda `CANCELAR_PROVA` + motor `Autorizacao.ADMIN`). **SEM assinatura desenhada (DP-2/ADR-074):** `exige_assinatura(acao)` + **`ACOES_ADMINISTRATIVAS={CANCELAR, REINICIAR_CICLO}`** (em `machine.py`, derivado das transições `ADMIN`); `executar` aceita `assinatura_imagem: bytes | None` → administrativas gravam `assinatura_ref` NULL (coluna já nullable — **sem migration**). Cancelar NÃO entra em `acoes-disponiveis` (C12). UI no detalhe (C08): botão de perigo na **mesma linha das ações de etiqueta** (`.acoes` em flex — 3 botões: Visualizar · Baixar · Cancelar) só com `podeCancelar` (resolvido no servidor por `can(perfil,"cancelar_prova")` — **não** existe `useAuthorization`) e `estaAtiva(status)`; `CancelarProvaModal` reusa `<MotionModal>` (motivo obrigatório, aviso de irreversibilidade), remontado por `key` (idempotência no `useState` init). Testes: `uv run pytest tests/unit/test_state_machine.py tests/unit/test_transicao_service.py tests/integration/test_cancelamento_endpoints.py tests/integration/test_transicoes_endpoints.py` (@db, mesma `TEST_DATABASE_URL`) + web `pnpm test`. **Sem migration nova** (head segue `0017`). Detalhes em `docs/cancelamento.md`.
>
> **Reinício de ciclo (W3-C15 — fecha a Wave 3):** ação ADMINISTRATIVA de reiniciar uma prova reprovada (3Studio, **só em `reprovada_vendedor`** — RN-006): volta a `criada`, **preserva rota e histórico** e **incrementa `ciclo_atual`** — a MESMA prova ganha um novo ciclo (mesmo código/QR/etiqueta). **Passa pelo motor do C11** (fonte única de status — ADR-076/062): endpoint **DEDICADO `POST /provas/{id}/reiniciar`** (`ReiniciarIn{idempotency_key}` → `ProvaDetalheOut`, prefixo real **sem `/api`**) por **`get_reinicio_service`** que gateia na **borda** por **`Recurso.REINICIAR_CICLO`** (flag admin; o recurso já existia nos 3 espelhos RBAC) e invoca o MESMO `ProvasTransicaoService.executar(acao=REINICIAR_CICLO, assinatura_imagem=None, motivo=None)`. **Incremento atômico (DP-1):** `incrementa_ciclo(acao)`+`ACOES_REINICIO` (`machine.py`, fonte única) → `executar` chama `ProvasRepositoryPort.incrementar_ciclo` (`UPDATE ... ciclo_atual+1 RETURNING`) **na mesma transação**, DEPOIS de registrar a movimentação (carimbada com o ciclo **pré-incremento** — DP-3, fecha o ciclo N; a Timeline do C13 separa os ciclos sem mudança). **SEM assinatura nem motivo (DP-2):** `REINICIAR_CICLO ∈ ACOES_ADMINISTRATIVAS` e `_REINICIAR` sem `exige_motivo` — só confirmação. **Acesso em 2 camadas** (borda `REINICIAR_CICLO` + motor `Autorizacao.ADMIN`). **Idempotente:** reenvio converge sem reincrementar (ciclo 2, nunca 3). **Migration `0018`:** `GRANT UPDATE (ciclo_atual)` em `provas` a `authenticated` (o grant de coluna do C11 não a incluía — senão "permission denied"); mirror `provas_grants.sql` com as 4 colunas. UI no detalhe (C08): botão **construtivo** `.btnReiniciar` na mesma linha das ações, só com `podeReiniciar` (servidor, `can(perfil,"reiniciar_ciclo")`) e `status === "reprovada_vendedor"`; `ReiniciarCicloModal` reusa `<MotionModal>` (sem motivo). Testes: `uv run pytest tests/unit/test_state_machine.py tests/unit/test_transicao_service.py tests/integration/test_reinicio_endpoints.py tests/integration/test_transicoes_endpoints.py` (@db, mesma `TEST_DATABASE_URL`) + web `pnpm test`. **Migration `0018` aplicada no Supabase real** (`alembic_version=0018`). Detalhes em `docs/reinicio-ciclo.md`. **Wave 3 concluída.**
>
> **Remediação da auditoria da Wave 3 (ADR-079):** fechou os 3 Médios + 3 Baixos endereçáveis do relatório `docs/audits/AUDITORIA-WAVE-3.md` (GO). **Head agora `0019`** — migration **`0019`** alinha `private.nomes_de_vendedores` ao escopo ampliado do Motorista (6 estados = `ESTADOS_ESCOPO_MOTORISTA`; M-02). **Endurecimento do role de runtime (M-01):** `DATABASE_URL` **deve** apontar para `rastreio_runtime` (NOBYPASSRLS) em staging/produção (`.env.example`/`docs/rbac.md`/`docs/provas.md`) e a app **recusa subir** com role superuser/`BYPASSRLS` nesses ambientes (`verificar_role_runtime_nao_privilegiado` no `lifespan`); **`FORCE RLS` NÃO** adotado (superuser o ignora; quebraria os `private.*`). Novo harness `tests/unit/test_equivalencia_rls_wave3.py` trava os espelhos RLS de `movimentacoes`/`assinaturas`/`rate_limit_contadores` (M-03). `audit/` excluído do ruff (scaffolding descartável).
>
> **Dashboard em tempo real (W4-C16 — abre a Wave 4):** visibilidade operacional **fiel ao design** (layout bento). **`GET /dashboard`** (prefixo real **sem `/api`**) → `DashboardOut` por **`get_dashboard_service`** (gate **`Recurso.DASHBOARD`** universal; o ESCOPO dos números é da **RLS de `provas`** — Matriz §7). **Agregação em CONSULTA ÚNICA** (`SqlAlchemyDashboardRepository` — RNF-022): os **5 contadores do design** (DP-1/ADR-080 — "seguir o design exato", desvio consciente do RF-015: sem "Reprovadas"/"Em Trânsito") + o **breakdown de "Atrasadas" por vendedor + total** numa **só ida ao banco** (mínimo de requisições — o delay do C09, o instante-limite e os nomes de `private.nomes_de_vendedores` são resolvidos DENTRO da query, sob `abrir_sessao_rls`); **sem N+1**. **Mapeamento contador→status** em **`domain/dashboard.py`** (DP-2/ADR-081, fonte única: Criadas hoje=`created_at` hoje; Com Vendedor=`retirada_vendedor`+`encaminhada_para_vendedor`; Aprovadas=`aprovada_vendedor`; **Na clicheria=`recebida_clicheria`** = "Concluídas"). **"Atrasada" (RN-008/DP-4/ADR-083):** prova ativa parada além do delay em **horas úteis** (seg–sex 07–18, fuso America/São_Paulo), base na **última movimentação**, exclui terminais — via **instante-limite único** (monotonicidade) computado pela função SQL **`private.instante_limite_atraso(now(), delay)`** (migration **`0020`**, schema `private` não exposto, `STABLE`/`SECURITY INVOKER`/`search_path=''`; espelho `migrations/rls/instante_limite_atraso.sql`); **sem polling** (RNF-021 — recalcula na carga + a cada evento Realtime). **Realtime (DP-5/ADR-084):** **UMA** subscription a `provas` → **refetch único debounced** da agregação escopada; carga **inicial SSR** (sem waterfall — RNF-001); queda degrada graciosamente. UI em **`/dashboard`** (`DashboardView` — bento; cards clicáveis → `/provas` pré-filtrado; card "Atrasadas" lista+total; atalhos role-aware **Escanear**/**Nova Prova** (DP-3/ADR-082, Nova Prova só 3Studio); count-up `<AnimatedCounter>` Framer Motion + `prefers-reduced-motion`; **sem Recharts**, responsivo). **C07 estendido (DP-6/ADR-085):** `GET /provas` aceita **`status` multi-valor** (`IN`; valor único intacto) + filtro **`atrasada`** (reusa `private.instante_limite_atraso` — mesma regra do Dashboard). Testes: `uv run pytest tests/unit/test_dashboard.py tests/integration/test_dashboard_endpoints.py tests/integration/test_provas_listagem_atrasada_endpoints.py` (@db, mesma `TEST_DATABASE_URL`) + web `pnpm test`. **Migration `0020` aplicada no Supabase real** (`alembic_version=0020`). Detalhes em `docs/dashboard.md`. **Wave 4 concluída.**
>
> **Relatórios gerenciais (W5-C17 — abre a Wave 5):** seção analítica **exclusiva do 3Studio** (flag `administrador` — Matriz §7 "Relatórios" ●○○○; `Recurso.RELATORIOS` já existia nos 3 espelhos RBAC, **sem PR de Matriz** — ADR-093). **4 abas** (Geral/3Studio/Vendedores/Clicheria) por **`GET /relatorios/{aba}`** + **`GET /relatorios/exportar?aba=…`** (prefixo real **sem `/api`**) via **`get_relatorios_service`** (gate **`Recurso.RELATORIOS`** → **403** ao não-admin em todas as abas E no export — DP-7). **Agregação server-side por aba** (`SqlAlchemyRelatoriosRepository`), poucas consultas **SET-BASED sem N+1** (RNF-022), **lazy** (só a aba ativa busca — DP-6), **SEM Realtime** (snapshot do período — recomputa na troca de filtro/aba). **Reuso do C16:** "Atrasada" pelos fragmentos `SQL_PREDICADO_ATRASADA`/`SQL_ULTIMO_EVENTO` **verbatim** + `private.instante_limite_atraso`; tempos em **horas úteis** pela **nova função `private.horas_uteis_entre(inicio, fim)`** (migration **`0021`**, schema `private` não exposto, `STABLE`/`SECURITY INVOKER`/`search_path=''`; espelho `migrations/rls/horas_uteis_entre.sql`). **Fórmulas (DP-3/ADR-088 — fonte única `domain/relatorios.py`):** tempo médio aprovação = **chegada ao vendedor → `aprovar`**; taxa = `reprov ÷ (aprov+reprov)`; **distribuição por rota = as 4 somando 100%** no período (critério §6.3); donut "Provas Ativas" = aguardando vendedor vs reprovada; **Devolvidas = reprovações no período (`acao=reprovar`)**; Top motivos = `movimentacoes.motivo WHERE acao='cancelar'`; Clicheria = perspectiva "rumo à clicheria" (`com_motorista_entrega_final`→`recebida_clicheria`); **atraso exibido em HORAS** (≠ "dias" da US-014). **População base (ADR-091):** o período recorta por **`created_at`**; demais filtros compõem; "nº requerimento" = coluna `provas.requerimento` (não o código PRV). **Export CSV (DP-5/ADR-089):** dataset da aba ativa preservando todos os campos + filtros, **UTF-8 com BOM + `;`** (Excel pt-BR), botão com caret. UI em **`/relatorios`** (`RelatoriosView` — tab bar + barra de filtros compartilhada na URL reusando o C07: De/Até + presets + Status + busca + toggle de rota 2-vias agrupado + Vendedor; **Recharts** barras+donut — **adicionado nesta wave** (ADR-092, contraria o que CLAUDE.md §4/prompt afirmavam: o C16 era chart-free); barras CSS p/ distribuição/ranking/motivos; `<AnimatedCounter>`; empty states; `error.tsx`; `prefers-reduced-motion`; responsivo). Testes: `uv run pytest tests/unit/test_relatorios.py tests/integration/test_relatorios_endpoints.py tests/integration/test_migrations.py` (@db, mesma `TEST_DATABASE_URL`) + web `pnpm test`. **Migration `0021` aplicada no Supabase real** (`alembic_version=0021`). Detalhes em `docs/relatorios.md`. **Aba Clicheria reconstruída em bento** (commit `9e3fbde`) — fidelidade visual completa. **C18 (Atalhos Rápidos) DESCARTADO por decisão de produto (ADR-095)** — a Wave 5 entrega só o C17; Relatórios é acessível só pela sidebar. **Próximo: re-auditar a Wave 5; se GO → W6-C19 (Camada de Animações).**
>
> **Camada de animações (W6-C19 — abre a Wave 6, frontend-only):** consolida a camada que já existia (tokens, `useReducedMotion` central, `<MotionModal>`, toasts, `<AnimatedCounter>`) e **adiciona** as page transitions (RF-023) e o **reveal de entrada ("elementos surgindo em cascata") em TODAS as telas, sob medida** (ADR-097). **Fundação:** tokens `STAGGER`/`REVEAL_OFFSET` (`lib/motion/tokens.ts`); **fábricas** `fadeRise`/`staggerContainer` (`lib/motion/variants.ts`, recebem `reduced`); **primitivas** `<PageTransition>` (extraída do AppShell, keyed por `pathname`, **enter-only** — ADR-098), `<Reveal>`, `<Stagger>`/`<StaggerItem>` (`components/ui/motion/`). **Gaps fechados (ADR-099):** `<MotionModal>` e o drawer do AppShell a **`DURATION.short` (250 ms — faixa RF-024)** + `AnimatePresence mode="wait"`; toasts com piso **≥ 4 s para erros** explícito (RF-027). **Duas técnicas:** (A) fábricas nos nós existentes (grids com `grid-area`, **tabelas** com **cap `STAGGER.maxItens`=12** — `provas`/`usuarios`); (B) primitivas em pilhas verticais (`escanear`, `Placeholder`). **Regras (CLAUDE.md §11):** só `transform`/`opacity` (GPU); **não** re-animar `<AnimatedCounter>` (RF-025) nem `<ProofTimeline>` (RF-026). Util de teste central **`@/test/motion` → `setReducedMotion`**. Testes da camada: `apps/web/src/lib/motion/variants.test.ts` + `components/ui/motion/motion.test.tsx` + `lib/motion/hooks.test.tsx`; **`pnpm test` 190 verdes (29 arq.)**, `tsc`/`eslint`/`next build` limpos. **Sem migration/RLS** (frontend-only; head segue `0021`). Detalhes em `docs/animations.md`.
>
> **Log de Auditoria (W6-C20 — fecha a Wave 6 e o backlog v1.0):** interface read-only **3Studio-only** sobre um **log imutável de TODAS as ações do sistema** (DP-1=C/ADR-100 — **expande o RNF-006**: o design pede mais que `movimentacoes`). Nova tabela própria **`audit_log`** (migration **`0022`**, append-only) que **unifica** transições (C11/C14/C15) + **criação** (C06) + **escaneamento** (C10) — **sem tocar `movimentacoes`** (a Timeline/C13 segue lendo de lá; `audit_log` é paralela/desacoplada). Campos de prova/ator **denormalizados** + `ip`/`origem` (best-effort, do `RequestIdMiddleware` via ContextVars — **nunca** decisão de acesso) + **chain de integridade** (`seq`/`prev_hash`/`hash` SHA-256). **Imutável em 2 camadas** (trigger `trg_audit_log_append_only` + sem GRANT UPDATE/DELETE). **Escrita só pela função `private.audit_log_append` (SECURITY DEFINER)** — INSERT direto revogado de `authenticated` (anti-forja): força o ator das claims, serializa o chain por `pg_advisory_xact_lock`, calcula o hash via **`private.audit_log_hash`** (fonte única; `extract(epoch)` + `SET lc_numeric='C'` p/ determinismo). **`private.audit_log_verificar`** recomputa o chain e localiza a 1ª quebra (tamper-evidence — DP-4; **sem HMAC** na v1.0). **Captura** = efeito colateral ATÔMICO via porta **`AuditLogPort`** (opcional `audit=None` nos testes): `criou_prova`/transições na MESMA transação (reenvio idempotente **não duplica**); `escaneou_qr` só no sucesso (404 não loga — anti-enum). **RLS** `audit_log_select_admin` = `app_is_admin()`. **Endpoints** `GET /auditoria` + `/auditoria/atores` + `POST /auditoria/verificar-integridade` por `get_auditoria_service` (gate `Recurso.LOG_AUDITORIA` → 403; **sem PR de Matriz** — o recurso já existia nos 3 espelhos). UI em **`/auditoria`** (`AuditoriaView` master-detail fiel ao design: filtros na URL/debounce reusando o C07, color-coding por tipo em `lib/auditoria/evento-labels.ts`, detalhe Ator/Setor/Prova/IP/Origem/Data + rodapé "Registro íntegro e imutável"/`sha256:…`, scroll infinito, **drill-in no mobile**, "Verificar integridade"; item **"Auditoria"** em `NAV_SECUNDARIA` 3Studio-only; animações C19). **Honestidade de dados:** ausente → "—" (sem IP/origem/hash falsos). Testes: `uv run pytest tests/unit/test_auditoria_dominio.py tests/unit/test_equivalencia_rls_audit_log.py tests/integration/test_audit_log_captura.py tests/integration/test_audit_log_integridade.py tests/integration/test_auditoria_endpoints.py tests/integration/test_rls_audit_log.py` (@db) + web `pnpm test`. **Migration `0022` aplicada no Supabase real** (`alembic_version=0022`; advisors sem item novo de `audit_log`). **Pendências adiadas:** eventos periféricos (login/config) + endurecimento HMAC. Detalhes em `docs/auditoria.md`. **Backlog v1.0 COMPLETO. Próximo: auditoria de fechamento da Wave 6 / revisão final.**
>
> **Realtime do dashboard (Etapa 3 da migração — fora do backlog v1.0):** substitui o Realtime do Supabase (ADR-113) por **SSE + Postgres `LISTEN/NOTIFY`**, custo R$ 0, **sem migration** (head segue `0023`). Uma mudança de prova (criar/transição/cancelar/reiniciar) emite `pg_notify('eventos_provas','mudou')` na MESMA transação (só no commit; **nunca** no reenvio idempotente) — porta **`EventBusPort`**/`PgNotifyEventBus` no molde do `AuditLogPort`, injetada nas 4 fábricas (ADR-115). O **`PgEventListener`** (1 por processo; conexão asyncpg **direta/sessão**, resiliente) alimenta o **`EventoHub`** (fan-out in-process; filas *bounded*+drop); ambos sobem no `lifespan` (`infrastructure/realtime.py` — ADR-117). **`GET /dashboard/stream`** (`StreamingResponse` `text/event-stream`, `X-Accel-Buffering: no`; ADR-116): auth por **cookie**, gate `DASHBOARD` em **sessão RLS curta** (`autorizar_stream_dashboard` — NÃO segura conexão `NullPool`), heartbeat ~20 s, **TTL derivado do `exp`** → evento `expira` → o cliente renova o cookie e reabre. O evento é **genérico** (nada por perfil trafega; o escopo é do refetch via RLS). Front: `apps/web/src/lib/api/eventos.ts` (`assinarDashboard`) + `dashboard-view.tsx` (debounce+jitter, `expira`→`/api/auth/refresh`→reabre). **Deploy on-prem:** o reverse proxy da rota do stream **deve** desligar buffering/gzip p/ `text/event-stream` e ter `proxy_read_timeout` > heartbeat (ADR-118). Testes: `uv run pytest tests/unit/test_realtime.py tests/unit/test_dashboard_stream.py tests/integration/test_realtime_notify.py tests/integration/test_dashboard_stream_endpoints.py` (@db) + web `pnpm test`. Detalhes em `docs/realtime.md`. **Migração Supabase→local COMPLETA (Banco + Auth + Realtime).**
...
- ❌ Não adicionar polling no dashboard — o live-update é **SSE (`GET /dashboard/stream`) + Postgres `LISTEN/NOTIFY`** (ADR-114..118, etapa 3 **CONCLUÍDA**, Sessão 31): evento **genérico** → refetch **debounced** escopado pela RLS. A restrição "sem polling" (RNF-021), suspensa pela ADR-113, está **REATIVADA** sob o SSE. Detalhes: `docs/realtime.md`.
